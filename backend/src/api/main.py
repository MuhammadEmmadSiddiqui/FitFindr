...
from ..services import GraphScreeningService
...
# CORS must be registered before routes so headers are added to every response
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
...
async def startup_event():
    ...
    if settings.LANGCHAIN_API_KEY:
        logger.info(
            f"LangSmith tracing active — project: {settings.LANGCHAIN_PROJECT} "
...
async def screen_resumes(
    ...
    try:
        ...
        screening_service = GraphScreeningService()
        ...
        if not results:
            raise HTTPException(
                status_code=422,
                detail="No resumes could be processed. Check that the files are valid PDFs or text files and the Groq API key is set."
            )
